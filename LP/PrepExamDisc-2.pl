...
converting(Expression, tree(Expression, null, null)):-integer(Expression), !.
...
% Substitue expression with another one
...
% sa pui in inorder nodurile root la subarbori cu nr impar de noduri cu 1 copil + sa generez arbore dfs al parcurgerii pe graf

:-dynamic parent/2.
:-dynamic children/2.
:-dynamic vis/1.

df_search2(X, _):-dfs2(X).
df_search2(_, _):-vis(X), findall(Y, parent(X, Y), L), assertz(children(X, L)), fail.
df_search2(X, T):-make_tree(X, T), listing(children(_, _)), retractall(vis(_)), retractall(parent(_, _)), retractall(children(_, _)).

dfs2(X):-assertz(vis(X)),
        is_edge(X, Y),
        not(vis(Y)),
        assertz(parent(X, Y)),
        dfs2(Y).
...
make_tree(X, t(X, [HT|R])):-children(X, [H|T]), !, make_tree(H, HT), cont_tree(T, R).
make_tree(X, t(X, [])).

cont_tree([H|T], [Tree|R]):-cont_tree(T, R), make_tree(H, Tree).
cont_tree([], []).
