...
df_search(X, _):-dfs(X).
df_search(_, L):-collect_v(L), retractall(vert(_)).
...
%topological sort on graph. Say if its possible
...
collect_stack([]):-retractall(vis(_)).
...
% Se dadea un graf (care era un poligon convex format din triunghiuri: asemanator cu problema de la seminar) 
prin reprezentare edge(X,Y), unde trebuie sa colectezi intr-o lista toate muchiile care nu sunt comune la mai multe triunghiuri.
...
post_array([10,4,2,9,20,50,100]).
post_array1([4, 5, 2, 3, 1]).
...
build_postorder_tree([H|T], t(H, L, nil)):-!, build_postorder_tree(T, L).
...
is_edge6(X, Y):-edge6(X, Y); edge6(Y, X).
...
df_search_multiple:-is_edge6(X, _),
                not(vertex4(X)),
                assertz(vertex4(X)),
                fail.
df_search_multiple:-vertex4(X),
                not(auxvis(X)),
                df_search4(X),
                fail.
df_search_multiple:-listing(component(_)), retractall(component(_)), retractall(vertex4(_)), retractall(vis4(_)), retractall(auxvis(_)).

df_search4(X):-dfs4(X).
df_search4(_):-findall(Y, vis4(Y), L),
                assertz(component(L)),
                retractall(vis4(_)).


dfs4(X):-assertz(vis4(X)),
        assertz(auxvis(X)),
        is_edge6(X, Y),
        not(vis4(Y)),
        dfs4(Y).
...
mst:-collect_edges(L), sort_by_weight(L, SL), make_tree(SL), listing(mst_edge(_, _, _)), retractall(mst_edge(_, _, _)), retractall(vis7(_)).

collect_edges(L):-findall(e(X, Y, Z), edge7(X, Y, Z), L).

sort_by_weight([e(X, Y, W)|T], R):-pivot_weight(W, T, Smalls, Bigs),
                        sort_by_weight(Smalls, SSmalls),
                        sort_by_weight(Bigs, SBigs),
                        append(SSmalls, [e(X, Y, W)|SBigs], R).
sort_by_weight([],[]).

pivot_weight(X, [e(A, B, W)|T], [e(A, B, W)|Smalls], Bigs):-
    W < X, !, pivot_weight(X, T, Smalls, Bigs).
pivot_weight(X, [H|T], Smalls, [H|Bigs]):- pivot_weight(X, T, Smalls, Bigs).
pivot_weight(_, [], [], []).
...
make_tree([e(X, Y, _)|T]):-
                ...
                make_tree(T).
make_tree([e(X, Y, W)|T]):-
                assertz(mst_edge(X, Y, W)),
                make_tree(T).
make_tree([]).


% Heads of lists at even levels

collect_even_depth_starts([], Last, Last, _).
collect_even_depth_starts([H|T], First, Last, D):-atomic(H), !, collect_even_depth_starts(T, First, Last, D).
collect_even_depth_starts([[H|T1]|T], [H|First], Last, D):-1 is D mod 2, !, D1 is D + 1, collect_even_depth_starts(T1, First, DLast, D1), collect_even_depth_starts(T, DLast, Last, D).
collect_even_depth_starts([[_|T1]|T], First, Last, D):-D1 is D + 1, collect_even_depth_starts(T1, First, DLast, D1), collect_even_depth_starts(T, DLast, Last, D).

collect_even_depth_starts(List, Res):-collect_even_depth_starts([List], Res, [], 1).
...
bipartite(L1, L2):-retractall(col(_,_)),
        edge8(X, Y),
        assertz(col(X,white)),
        assertz(col(Y,black)),
        call_bfs(L1, L2).
...
call_bfs(L1, L2):-
        bfs, !,
        findall(X, col(X, black), L1),
        findall(Y, col(Y, white), L2).
...
check(X, C):-col(X, C), !, fail.
check(X, _):-col(X, _).
check(X, C):-neg(C, C1), assertz(col(X, C1)).

neg(black, white).
neg(white, black).

% diameter of ternary incomplete treee

diam_tern(nil, -1, 0).
diam_tern(t(_, L, M, R), D, H):-
                diam_tern(L, DL, HL),
                diam_tern(M, DM, HM),
                diam_tern(R, DR, HR),
                DMR is HM - 1 + HR - 1 + 2,
                DLR is HL - 1 + HR - 1 + 2,
                DLM is HL - 1 + HM - 1 + 2,
                max_list([HL, HM, HR], Haux),
                H is Haux + 1,
                max_list([DL, DM, DR, DMR, DLR, DLM], D).